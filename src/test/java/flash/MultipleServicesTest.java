...
public class MultipleServicesTest {
    ...
    private static FlashTestUtil firstClient;
    private static FlashTestUtil secondClient;

    @BeforeClass
    public static void setup() throws Exception {
        firstClient = new FlashTestUtil(4567);
        secondClient = new FlashTestUtil(1234);
    ...
    public void testGetHello() throws Exception {
        FlashTestUtil.UrlResponse response = firstClient.doMethod("GET", "/hello", null);
    ...
    public void testGetRedirectedHi() throws Exception {
        FlashTestUtil.UrlResponse response = secondClient.doMethod("GET", "/hi", null);
        Assert.assertEquals(200, response.status);
        Assert.assertEquals("Hello World!", response.body);
    }
    ...
    public void testGetUniqueForSecondWithFirst() throws Exception {
        FlashTestUtil.UrlResponse response = firstClient.doMethod("GET", "/uniqueforsecond", null);
        Assert.assertEquals(404, response.status);
    }

    @Test
    public void testGetUniqueForSecondWithSecond() throws Exception {
        FlashTestUtil.UrlResponse response = secondClient.doMethod("GET", "/uniqueforsecond", null);
        Assert.assertEquals(200, response.status);
    ...
    public void testStaticFileCssStyleCssWithFirst() throws Exception {
        FlashTestUtil.UrlResponse response = firstClient.doMethod("GET", "/css/style.css", null);
        Assert.assertEquals(404, response.status);
    }

    @Test
    public void testStaticFileCssStyleCssWithSecond() throws Exception {
        FlashTestUtil.UrlResponse response = secondClient.doMethod("GET", "/css/style.css", null);
        Assert.assertEquals(200, response.status);
