...
public class StaticFilesTest {
    ...
    public void testMimeTypes() throws Exception {
        ...
        Assert.assertEquals("image/png", doGet("/img/flashlogo.png").headers.get("Content-Type"));
        Assert.assertEquals("image/svg+xml", doGet("/img/flashlogo.svg").headers.get("Content-Type"));
        Assert.assertEquals("application/octet-stream", doGet("/img/flashlogoPng").headers.get("Content-Type"));
        Assert.assertEquals("application/octet-stream", doGet("/img/flashlogoSvg").headers.get("Content-Type"));
    ...
    public void testStaticFileCssStyleCss() throws Exception {
        FlashTestUtil.UrlResponse response = doGet("/css/style.css");
    ...
    public void testStaticFilePagesIndexHtml() throws Exception {
        FlashTestUtil.UrlResponse response = doGet("/pages/index.html");
        Assert.assertEquals(200, response.status);
    ...
    public void testStaticFilePageHtml() throws Exception {
        FlashTestUtil.UrlResponse response = doGet("/page.html");
        Assert.assertEquals(200, response.status);
    ...
    public void testDirectoryTraversalProtectionLocal() throws Exception {
        String path = "/" + URLEncoder.encode("..\\flash\\", "UTF-8") + "Flash.class";
        FlashTestUtil.UrlResponse response = doGet(path);
    ...
    public void testExternalStaticFile() throws Exception {
        FlashTestUtil.UrlResponse response = doGet("/externalFile.html");
        Assert.assertEquals(200, response.status);
    ...
    private static void testGet() throws Exception {
        FlashTestUtil.UrlResponse response = testUtil.doMethod("GET", "/hello", "");
    ...
    public void testExceptionMapping404() throws Exception {
        FlashTestUtil.UrlResponse response = doGet("/filethatdoesntexist.html");

        Assert.assertEquals(404, response.status);
        Assert.assertEquals(NOT_FOUND_BRO, response.body);
    }

    private FlashTestUtil.UrlResponse doGet(String fileName) throws Exception {
